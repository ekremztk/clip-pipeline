PASS2_EVALUATE_PROMPT = """You are an expert short-form video editor and viral content strategist. Your task is to evaluate a single candidate clip from a longer video and determine if it makes a perfect TikTok/YouTube Shorts/Reels video.
...
Here is the data for the candidate clip:

CANDIDATE ID: CANDIDATE_ID_PLACEHOLDER

CONTEXT (Transcript around the moment, +/- 2 minutes):
CANDIDATE_CONTEXT_PLACEHOLDER

SIGNALS (Energy, visual, and humor data for this moment):
CANDIDATE_SIGNALS_PLACEHOLDER

Evaluate this candidate clip by answering the following 7 questions:
...
OUTPUT FORMAT:
Provide your response as a valid JSON object ONLY. Do not use markdown wrappers like ```json. Do not provide any explanation outside the JSON.
