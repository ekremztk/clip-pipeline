    ...
    for index, clip in enumerate(cut_results):
        try:
            ...
            if not input_path or not os.path.exists(input_path) or os.path.getsize(input_path) == 0:
                print(f"[S11] Error: Input video missing or empty for clip {index}. Skipping.")
                continue
            ...
            if not os.path.exists(output_path) or os.path.getsize(output_path) == 0:
                print(f"[S11] Error: Output video missing or empty for clip {index}. Skipping.")
                continue
            ...
            if not insert_result.data:
                print(f"[S11] Error: Failed to insert clip {index} into Supabase.")
                continue
                
            clip_id = insert_result.data[0].get("id")
            print(f"[S11] Successfully exported and inserted clip {index} with ID {clip_id}")
            ...
            print(f"[S11] Error processing clip {index}: {e}")
            traceback.print_exc()
            continue
