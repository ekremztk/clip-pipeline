...
export default function LoginPage() {
    ...
    const getStrengthColor = (i: number) => {
        if (strength === 0) return { background: 'rgba(250,249,245,0.08)' };
        if (strength === 1) return i === 0 ? { background: '#ef4444' } : { background: 'rgba(250,249,245,0.08)' };
        if (strength === 2) return i < 2 ? { background: '#f97316' } : { background: 'rgba(250,249,245,0.08)' };
        if (strength === 3) return i < 3 ? { background: '#eab308' } : { background: 'rgba(250,249,245,0.08)' };
    ...
    const handleSignUp = async (e: React.FormEvent) => {
        ...
        if (data.user && !data.session) { setSignupEmail(email); setShowOtpScreen(true); startCooldown(); }
        setLoading(false);
    };
    ...
    const handleVerifyOtp = async (e: React.FormEvent) => {
        ...
        const { data, error } = await supabase.auth.verifyOtp({ email: signupEmail, token: otpCode, type: "signup" });
        if (error) { setOtpError("Invalid or expired code. Please try again."); setOtpLoading(false); return; }
        if (data.session) { router.push("/dashboard"); }
        else { setOtpError("Verification failed. Please try again."); setOtpLoading(false); }
    };

    const handleResend = async () => {
        if (resendCooldown > 0) return;
        setOtpError(null); setOtpSuccess(null);
        const { error } = await supabase.auth.resend({ type: "signup", email: signupEmail });
        if (error) { setOtpError(error.message); } else { setOtpSuccess("New code sent."); startCooldown(); }
    };
    ...
    const resetState = () => {
        setError(null); setEmail(""); setPassword(""); setFullName(""); setConfirmPassword(""); setTermsAccepted(false);
    };
    ...
    if (showOtpScreen) {
        return (
            <div style={{ background: '#141413' }} className="min-h-screen flex items-center justify-center p-4">
                    ...
                    <div style={{ background: '#181817', border: '1px solid rgba(250,249,245,0.07)' }} className="rounded-2xl p-6">
                        ...
                        <form onSubmit={handleVerifyOtp} className="space-y-4">
                            <div>
                                ...
                                <input
                                    ...
                                    maxLength={6}
                                    value={otpCode}
                                    onChange={(e) => { const val = e.target.value.replace(/\D/g, ""); setOtpCode(val); setOtpError(null); }}
                                    disabled={otpLoading}
                                    autoFocus
                                    placeholder="000000"
                                    style={inputStyle}
                                    className="w-full rounded-xl px-3 py-3 text-lg text-center tracking-[0.5em] placeholder:text-[rgba(250,249,245,0.2)] focus:outline-none transition-colors disabled:opacity-50 font-mono"
                                />
                            </div>
                        ...
                        <div style={{ borderTop: '1px solid rgba(250,249,245,0.07)' }} className="flex items-center justify-between mt-5 pt-4">
                            ...
                            <button
                                ...
                                style={{ color: resendCooldown > 0 ? 'rgba(250,249,245,0.2)' : 'rgba(250,249,245,0.5)' }}
                                className="text-xs hover:text-[#faf9f5] transition-colors disabled:cursor-not-allowed"
                            >
    ...
    return (
        <div style={{ background: '#141413' }} className="min-h-screen flex items-center justify-center p-4">
            <div className="w-full max-w-[400px]">
                ...
                <div style={{ background: '#181817', border: '1px solid rgba(250,249,245,0.07)' }} className="rounded-2xl p-6">
                    {/* Tabs */}
                    <div style={{ borderBottom: '1px solid rgba(250,249,245,0.07)' }} className="flex mb-6">
                        <button
                            onClick={() => { if (activeTab !== "signin") { setActiveTab("signin"); resetState(); } }}
                            style={{ color: activeTab === "signin" ? '#faf9f5' : 'rgba(250,249,245,0.3)' }}
                            className="flex-1 pb-3 text-sm font-medium transition-colors relative hover:text-[#faf9f5]"
                        >
                            Sign In
                            {activeTab === "signin" && (
                                <div style={{ background: '#faf9f5' }} className="absolute bottom-0 left-0 right-0 h-[2px] rounded-full" />
                            )}
                        </button>
                        <button
                            onClick={() => { if (activeTab !== "signup") { setActiveTab("signup"); resetState(); } }}
                            style={{ color: activeTab === "signup" ? '#faf9f5' : 'rgba(250,249,245,0.3)' }}
                            className="flex-1 pb-3 text-sm font-medium transition-colors relative hover:text-[#faf9f5]"
                        >
                            Create Account
                            {activeTab === "signup" && (
                        ...
                        <form onSubmit={handleSignIn} className="space-y-4">
                            <div>
                                ...
                                <input
                                    ...
                                    disabled={loading}
                                    style={inputStyle}
                                    className="w-full rounded-xl px-3 py-2.5 text-sm placeholder:text-[rgba(250,249,245,0.2)] focus:outline-none transition-colors disabled:opacity-50"
                        ...
                        <form onSubmit={handleSignUp} className="space-y-4">
                            ...
                            <div>
                                ...
                                <div className="flex gap-1 mt-2">
                                    {[0, 1, 2, 3].map((i) => (
                                        <div key={i} style={{ height: '4px', flex: 1, borderRadius: '2px', transition: 'background-color 0.3s', ...getStrengthColor(i) }} />
                                    ))}
                                </div>
                            </div>
                            <div>
                    ...
                    <button
                        ...
                        className="w-full rounded-xl text-sm font-medium py-2.5 transition-colors flex items-center justify-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed hover:border-[rgba(250,249,245,0.2)]"
                    >
